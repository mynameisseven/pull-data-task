...
public class BatchController {
    ...
    @GetMapping("/batch/allList")
    public void batchList() {

        List<PrimaryBatch> batchlist = primaryBatchMapper.getAll();
        secondBatchService.insertBatchList(batchlist);
        logger.info("------batch---all--导入数据结束---------------");
    }

    @GetMapping("/batchNew/allList")
    public void batchNewList() {

        List<PrimaryBatchNew> batchlist = primaryBatchNewMapper.getAll();
        secondBatchNewService.insertBatchList(batchlist);
        logger.info("---batchNew----all----导入数据结束---------------");
    }

    @GetMapping("/terminalBatch/allList")
    public void terminalBatchList() {

        List<PrimaryTerminalBatch> batchlist = primaryTerminalBatchMapper.getAll();
        secondTerminalBatchService.insertBatchList(batchlist);
        logger.info("-----------terminalBatch--all--导入数据结束---------------");
    }

    @GetMapping("/batch/yesterdayList")
    public void batchYesterdayList() {

        List<PrimaryBatch> batchlist = primaryBatchMapper.getYesterdayList();
        secondBatchService.insertBatchList(batchlist);
        logger.info("----batch----yesterday---导入数据结束---------------");
    }

    @GetMapping("/batchNew/yesterdayList")
    public void batchNewYesterdayList() {

        List<PrimaryBatchNew> batchlist = primaryBatchNewMapper.getYesterdayList();
        secondBatchNewService.insertBatchList(batchlist);
        logger.info("----batchNew----yesterday---batchNew导入数据结束---------------");
    }

    @GetMapping("/terminalBatch/yesterdayList")
    public void terminalBatchYesterdayList() {

        List<PrimaryTerminalBatch> batchlist = primaryTerminalBatchMapper.getYesterdayList();
        secondTerminalBatchService.insertBatchList(batchlist);
        logger.info("---terminalBatch---yesterdayList-----导入数据结束---------------");
    }
